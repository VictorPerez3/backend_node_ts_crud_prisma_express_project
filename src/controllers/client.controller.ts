...
export const getAll = async (req: Request, res: Response) => {
  try {
    ...
    const clientToken = await authServices.decodedToken(
...
export const createClient = async (req: Request, res: Response) => {
  try {
    ...
    //Criado cliente
    const clientCreate = await clientRepository.create(req.body);
...
export const updateByIdClient = async (req: Request, res: Response) => {
  try {
    ...
    const clientById = await clientRepository.findClientByIdRepository(
      parseInt(id),
    );
    ...
    if (!clientById) {
      ...
      if (clientById.agencyId === clientToken.agencyId) {
        ...
        const clientUpdateById = await clientRepository.update(
          parseInt(id),
          req.body,
        );
        res.json({
  ...
  try {
    ...
    if (!clientById) {
      ...
      if (clientById.agencyId === clientToken.agencyId) {
        ...
        const clientDeleteById = await clientRepository.destroy(parseInt(id));
        res.json({
          success: true,
